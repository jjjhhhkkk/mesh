...
(function() {
    ...
    $.fn.prettyTextDiff.createHTML = function(diff) {
        var data, html, operation, pattern_Backofficep, pattern_gt, pattern_lt, pattern_para, text;
        html = [];
        pattern_Backofficep = /&/g;
        pattern_lt = /</g;
        pattern_gt = />/g;
        pattern_para = /\n/g;
        operation = diff[0], data = diff[1];
        text = data.replace(pattern_Backofficep, '&amp;').replace(pattern_lt, '&lt;').replace(pattern_gt, '&gt;').replace(pattern_para, '<br>');
        switch (operation) {
            case DIFF_INSERT:
                return '<ins>' + text + '</ins>';
            case DIFF_DELETE:
